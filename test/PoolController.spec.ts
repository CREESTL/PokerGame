...
const initTotalWinningsMilestones = [
  0, 20000000000, 60000000000, 100000000000, 140000000000, 180000000000,
  220000000000,
];
const initBonusPercentMilestones = [1, 2, 4, 6, 8, 10, 12];
const changedTotalWinningsMilestones = [
  "10",
  "120000000000",
  "160000000000",
  "1100000000000",
...
describe("Poker", () => {
  ...
  beforeEach("deploy fixture", async () => {
    ({ oracle, poker, poolController } = await loadFixture(contractsFixture));
  });
  ...
  describe("check getters and setters", async () => {
    ...
    it("setBonusPercentMilestones ", async () => {
      let contractBonusMilestones = dumbArrayToString(
        await poolController.getBonusPercentMilestones()
      );
      expect(contractBonusMilestones).to.equal(
        dumbArrayToString(initBonusPercentMilestones)
      );
      await poolController.setBonusPercentMilestones(
        changedBonusPercentMilestones
      );
      contractBonusMilestones = dumbArrayToString(
        await poolController.getBonusPercentMilestones()
      );
      expect(contractBonusMilestones).to.equal(
        dumbArrayToString(changedBonusPercentMilestones)
      );
    });

    it("getTotalWinningsMilestones ", async () => {
      let contractTotalWinningsMilestones = dumbArrayToString(
        await poolController.getTotalWinningsMilestones()
      );
      expect(contractTotalWinningsMilestones).to.equal(
        dumbArrayToString(initTotalWinningsMilestones)
      );
      await poolController.setTotalWinningsMilestones(
        changedTotalWinningsMilestones
      );
      contractTotalWinningsMilestones = dumbArrayToString(
        await poolController.getTotalWinningsMilestones()
      );
      expect(contractTotalWinningsMilestones).to.equal(
        dumbArrayToString(changedTotalWinningsMilestones)
      );
    });
  ...
  describe("check whitelist logic", async () => {
    ...
    it("addToWhitelist should fail", async () => {
      await expect(poolController.addToWhitelist(wallet.address)).to.be
        .reverted;
    });
  ...
  describe("check misc reverts", async () => {
    ...
    it("check onlyOracleOperator modifier", async () => {
      // @ts-ignore
      await expect(poolController.takeOracleFee({ from: other.address })).to.be
        .reverted;
    });

    it("check deposit allowed only for whitelisted addresses", async () => {
      // @ts-ignore
      await expect(poolController.deposit({ from: other.address })).to.be
        .reverted;
    });
  });
